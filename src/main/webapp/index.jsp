...
<head>
    ...
    <!--
        ...
        以/开始的相对路径，找资源，以服务器的路径为基准，需要加上项目名。
    -->
...
<div class="modal fade" id="empUpdateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <script type="text/javascript">
        ...
        //页面加载完成以后，直接发送AJAX请求，获取员工和分页信息，并先去首页
        $(function () {
            //去首页
            to_page(1);
        });
        ...
        function getDepts(ele) {
            ...
            $.ajax({
                ...
                type: "GET",
                success: function (result) {
                    //result即：{"code":100,"msg":"处理成功","extend":{"depts":[{"deptId":1,"deptName":"开发部"},{"deptId":2,"deptName":"测试部"}]}}
                    //console.log(result);
                    //显示部门信息在下拉列表中
        ...
        function validate_add_form() {
            ...
            if (!regName.test(empName)) {
                //alert("用户名必须是6~16位字母和数字的组合或者2~5位汉字");
                show_validate_msg("#empName_add_input", "error", "用户名必须是6~16位字母和数字的组合或者2~5位汉字");
                return false;
            } else {
        ...
        function show_validate_msg(ele, status, msg) {
            //清空当前元素之前校验结果的对应样式和提示信息
            $(ele).parent().removeClass("has-success has-error");
            $(ele).next("span").text("");
            //显示当前元素最新校验结果的对应样式和提示信息
            if ("success" == status) {
        ...
        //编辑按钮创建之前就绑定click是绑定不上的
        //1).可以在创建按钮的时候绑定click；2).绑定live；3).jQuery新版没有live，使用on进行替代
